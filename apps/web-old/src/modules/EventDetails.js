...
export function EventDetails() {
    ...
    return (_jsxs("div", { className: "w-full max-w-2xl mx-auto px-4 sm:px-6", children: [_jsxs("div", { className: "flex items-center gap-2 sm:gap-3 flex-wrap", children: [_jsxs("h1", { className: "text-2xl sm:text-3xl font-bold text-gray-800 dark:text-gray-100", children: [categoryInfo.emoji, " ", event.name || event.title] }), _jsxs("span", { className: "text-gray-600 dark:text-gray-300", children: ["(", countdown, ")"] }), _jsx(Link, { to: `/edit/${event.id}`, className: "bg-blue-200 hover:bg-blue-300 rounded px-2 py-1 text-sm", children: "Edit" })] }), _jsx("div", { className: `inline-block px-2 py-1 rounded text-xs mt-1 ${categoryInfo.color}`, children: categoryInfo.label }), _jsx("div", { className: "mt-2", children: _jsx("button", { className: "text-sm text-blue-600 hover:underline", onClick: () => setShowShare(true), children: "Share" }) }), event.lastEdited && (_jsxs("div", { className: "text-xs text-gray-600 mt-1", children: ["Last edited: ", new Date(event.lastEdited).toLocaleString()] })), event.description && (_jsx("p", { className: "text-gray-600 mt-1", children: event.description })), _jsxs("div", { className: "mt-6", children: [_jsx("h2", { className: "text-xl sm:text-2xl font-semibold mb-2 text-gray-800 dark:text-gray-100", children: "Preparation Steps" }), !showAddTaskForm && (_jsx("button", { className: "px-4 py-2 min-h-[44px] w-full sm:w-auto rounded bg-blue-600 text-white hover:bg-blue-700 mb-4", onClick: () => setShowAddTaskForm(true), children: "Add a Preparation Step" })), showAddTaskForm && (_jsx("div", { className: "mb-6", children: _jsx(TaskForm, { onSubmit: handleAddTask, onCancel: () => setShowAddTaskForm(false), title: "Add a Preparation Step" }) })), _jsxs("ul", { className: "space-y-3", children: [event.tasks.map((t) => {
                                ...
                                return (_jsx("li", { className: "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded p-3", children: editingTaskId === t.id ? (_jsx(TaskForm, { initialValues: t, onSubmit: handleUpdateTask, onCancel: handleCancelEdit, title: "Edit Preparation Step" })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx("input", { type: "checkbox", checked: t.done || t.completed, onChange: (e) => {
                                                            e.stopPropagation();
                                                            toggleTask(t.id, event.id);
                                                        } }), _jsxs("div", { className: "flex items-center gap-2 flex-1", children: [_jsx("span", { className: `${(t.completed || t.done) ? "line-through text-gray-500" : "text-gray-800 dark:text-gray-100"}`, children: t.text || t.title }), t.completionDate && (_jsx("span", { title: `Due in ${daysLeft} day${daysLeft === 1 ? "" : "s"}${!t.reminderEnabled ? " (reminder disabled)" : ""}`, className: `text-sm ${!t.reminderEnabled
                                                                ...
                                                                }, children: "\uD83D\uDDD1\uFE0F Delete" })] })] }), _jsxs("div", { className: "text-sm text-gray-600 dark:text-gray-300 space-y-1", children: [t.owner && _jsxs("div", { children: ["Owner: ", t.owner] }), t.completionDate && (_jsxs("div", { className: "flex items-center gap-2", children: [_jsxs("span", { children: ["Target: ", new Date(t.completionDate).toLocaleDateString()] }), _jsx("span", { className: `font-medium ${getTaskCountdownColor(daysLeft)}`, children: countdownText })] })), (t.completed || t.done) && t.completionDate && (_jsxs("div", { className: "text-green-600", children: ["\u2705 Completed: ", new Date(t.completionDate).toLocaleDateString()] }))] })] })) }, t.id));
                            }), event.tasks.length === 0 && (_jsx("li", { className: "text-sm text-gray-600", children: "No tasks yet." }))] })] }), isPast && (_jsxs("section", { className: "mt-6 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-md", children: [_jsx("h3", { className: "text-lg font-semibold mb-2", children: "\uD83D\uDCDD Reflection" }), !event.reflection ? (_jsxs(_Fragment, { children: [_jsx("textarea", { className: "w-full p-3 border rounded-md bg-gray-50 dark:bg-gray-700 text-sm", placeholder: "How was this moment? Share your thoughts...", value: newReflection, onChange: (e) => setNewReflection(e.target.value) }), _jsxs("div", { className: "mt-3 flex items-center gap-2", children: [_jsx("input", { type: "file", accept: "image/*", onChange: handleReflectionPhotoSelect }), _jsx("button", { onClick: handleSaveReflection, className: "px-3 py-2 min-h-[40px] bg-indigo-500 text-white rounded-md", children: "Save Reflection" })] })] })) : (_jsxs("div", { children: [_jsx("p", { className: "text-gray-700 dark:text-gray-300 whitespace-pre-line", children: event.reflection }), event.reflectionPhoto && (_jsx("img", { src: event.reflectionPhoto, alt: "Moment Memory", className: "mt-3 rounded-lg w-full max-w-md" }))] }))] })), showShare && (_jsx("div", { className: "fixed inset-0 bg-black/40 flex items-center justify-center z-50", children: _jsxs("div", { className: "bg-white dark:bg-gray-800 rounded-lg p-4 w-full max-w-sm shadow mx-3", children: [_jsx("h3", { className: "font-semibold mb-2", children: "Share this Moment" }), _jsx("input", { type: "email", value: shareEmail, onChange: (e) => setShareEmail(e.target.value), placeholder: "Enter collaborator email", className: "border rounded w-full px-3 py-2 mb-3" }), _jsxs("div", { className: "flex justify-end gap-2", children: [_jsx("button", { className: "text-gray-600", onClick: () => setShowShare(false), children: "Cancel" }), _jsx("button", { className: "bg-blue-600 text-white rounded px-4 py-2", onClick: async () => {
                                        if (!shareEmail.trim())
                                            return;
