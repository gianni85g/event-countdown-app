...
export function Home() {
    ...
    const handleNotificationClick = async (notification) => {
        if (!supabase?.from || notification.read)
            return;
        try {
    ...
    if (loading) {
        return (_jsx("div", { className: "flex items-center justify-center py-20", children: _jsxs("div", { className: "text-center", children: [_jsx("p", { className: "text-gray-500 dark:text-gray-400 text-lg", children: "Loading moments..." }), _jsx("div", { className: "mt-4 animate-pulse", children: _jsx("div", { className: "h-2 bg-gray-200 dark:bg-gray-700 rounded w-48 mx-auto" }) })] }) }));
    }
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.3 }, className: "text-center mt-2", children: [_jsx("h1", { className: "font-semibold text-3xl text-gray-800 dark:text-gray-100", children: "\u2728 My Moments" }), _jsx("p", { className: "mt-1 text-gray-600 dark:text-gray-300", children: "Treasure your memories. Plan your next adventures." })] }), _jsxs("div", { className: "flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4", children: [_jsxs("div", { className: "flex gap-2", children: [_jsx("button", { onClick: () => setMomentFilter("all"), className: `px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 hover:scale-[1.05] active:scale-[0.95] focus:ring-2 focus:ring-blue-400 focus:outline-none ${momentFilter === "all"
                                    ...
                                    ? "bg-blue-500 dark:bg-blue-600 text-white shadow-md"
                                    : "bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300"}`, children: "Shared with Me" })] }), user && (_jsxs("div", { className: "relative", ref: notificationsRef, children: [_jsxs("button", { onClick: () => setShowNotifications(!showNotifications), className: "relative p-2 rounded-lg text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 focus:ring-2 focus:ring-blue-400 focus:outline-none", "aria-label": "Notifications", children: [_jsx(Bell, { className: "w-6 h-6" }), unreadCount > 0 && (_jsx(motion.span, { initial: { scale: 0 }, animate: { scale: 1 }, className: "absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-semibold", children: unreadCount > 9 ? "9+" : unreadCount })), typeof import.meta !== 'undefined' && import.meta.env?.MODE === 'development' && (_jsxs("span", { className: "absolute -bottom-6 left-0 text-xs text-gray-400", children: ["Total: ", notifications.length] }))] }), showNotifications && (_jsxs(motion.div, { initial: { opacity: 0, y: -10 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -10 }, className: "absolute right-0 top-12 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 w-80 max-h-96 overflow-hidden z-50", children: [_jsxs("div", { className: "p-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between", children: [_jsx("h3", { className: "font-semibold text-gray-800 dark:text-gray-100", children: "Notifications" }), typeof import.meta !== 'undefined' && import.meta.env?.MODE === 'development' && (_jsx("button", { onClick: async () => {
                                                    if (!user?.email || !supabase?.from)
                                                        return;
                                                    const normalizedEmail = user.email.toLowerCase().trim();
                                                ...
                                                }, className: "text-xs px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600", title: "Test notification query", children: "Test" }))] }), _jsx("div", { className: "max-h-80 overflow-y-auto", children: notifications.length === 0 ? (_jsx("div", { className: "p-4 text-center text-sm text-gray-500 dark:text-gray-400", children: "No notifications" })) : (notifications.map((n) => (_jsxs("div", { onClick: () => handleNotificationClick(n), className: `p-3 border-b border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${!n.read ? "bg-blue-50 dark:bg-blue-900/20" : ""}`, children: [_jsx("p", { className: "text-sm text-gray-800 dark:text-gray-100", children: n.message }), n.link && (_jsx("a", { href: n.link, onClick: (e) => {
                                                        e.preventDefault();
                                                        navigate(n.link);
                                                        setShowNotifications(false);
                                                    }, className: "text-blue-600 dark:text-blue-400 hover:underline text-xs mt-1 block", children: "View Moment \u2192" })), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-400 mt-1", children: new Date(n.created_at).toLocaleString() })] }, n.id)))) })] }))] }))] }), _jsx("div", { className: "flex flex-wrap gap-2 justify-center mb-4", children: categories.map(cat => (_jsx("button", { onClick: () => setFilter(cat), className: `py-2 px-3 text-sm md:text-base rounded-full font-medium transition-all duration-200 hover:scale-[1.05] active:scale-[0.95] focus:ring-2 focus:ring-blue-400 focus:outline-none ${filter === cat
                        ? 'bg-blue-500 dark:bg-blue-600 text-white shadow-md'
                        ...
                        ? 'All'
                        : cat.charAt(0).toUpperCase() + cat.slice(1) }, cat))) }), _jsx("h2", { className: "text-xl font-semibold text-gray-800 dark:text-gray-100 mb-3", children: momentFilter === "mine" ? "My Moments" : momentFilter === "shared" ? "Shared with Me" : "All Moments" }), _jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4", children: [_jsx(AnimatePresence, { children: visibleEvents.filter((m) => !isPast(m)).map((e) => {
                            ...
                            return (_jsx("div", { className: "rounded-2xl shadow-lg hover:shadow-xl transition-all border border-transparent bg-gradient-to-r from-pink-200 to-indigo-200 p-[1px] relative", children: _jsxs(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -20 }, transition: { duration: 0.3 }, className: `bg-white dark:bg-gray-800 rounded-2xl p-4 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 ${e.status === "pending" ? "opacity-60" : "opacity-100"}`, children: [isPast(e) && (_jsx("span", { className: "absolute top-2 right-3 text-xs px-2 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 rounded-full", children: "\u23F3 Past Due" })), _jsxs("div", { className: "flex items-start justify-between mb-2", children: [_jsxs("div", { className: "flex-1", children: [_jsxs("h3", { className: "text-lg font-semibold text-gray-800 dark:text-gray-100 mb-1", children: [categoryInfo.emoji, " ", e.name || e.title] }), _jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx("span", { className: "text-sm text-gray-500", children: new Date(e.date).toLocaleDateString() }), _jsx("span", { className: `text-xs font-medium px-2 py-1 rounded-full capitalize ${e.category === "work"
                                                                        ...
                                                                        : e.category === "birthday"
                                                                            ...
                                                                            : e.category === "travel"
                                                                                ...
                                                                                : e.category === "education"
                                                                                    ...
                                                                                    : "bg-rose-100 text-rose-800"}`, children: e.category })] })] }), _jsx("div", { className: `text-sm font-medium ${urgent ? "text-red-600" : "text-gray-600"}`, children: getCountdown(e.date) })] }), e.description && (_jsx("p", { className: "text-sm text-gray-600 dark:text-gray-300 mt-1", children: e.description })), _jsxs("p", { className: "text-sm text-gray-600 dark:text-gray-300 mt-2 flex items-center justify-between", children: [_jsxs("span", { children: ["\uD83D\uDCCB ", total, " ", total === 1 ? "preparation step" : "preparation steps"] }), total > 0 ? (_jsxs("span", { children: [total - completed, " open \u2022 ", completed, " done", completed === total && _jsx("span", { className: "ml-1", children: "\uD83C\uDF89" })] })) : (_jsx("span", { className: "text-gray-400", children: "no steps yet \u2014 start preparing \uD83C\uDF3F" }))] }), e.status === "pending" ? (_jsxs("div", { className: "mt-4 flex gap-2", children: [_jsx("button", { onClick: async () => {
                                                    ...
                                                    }, disabled: processingMomentId === e.id, className: "px-4 py-2 rounded-md bg-green-500 dark:bg-green-600 text-white hover:bg-green-600 dark:hover:bg-green-700 transition-all duration-200 hover:scale-[1.05] active:scale-[0.95] hover:shadow-md focus:ring-2 focus:ring-green-400 focus:outline-none text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed", children: processingMomentId === e.id ? "Processing..." : "Accept" }), _jsx("button", { onClick: async () => {
                                                                    ...
                                                                    task.eventCategory === "birthday" ? "bg-pink-200 text-pink-800" :
                                                                        task.eventCategory === "travel" ? "bg-teal-200 text-teal-800" :
                                                                            task.eventCategory === "education" ? "bg-indigo-200 text-indigo-800" :
                                                                                "bg-rose-200 text-rose-800"}`, children: [categoryInfo.emoji, " ", task.eventTitle] }), _jsx("div", { className: "mt-2", children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "font-medium text-gray-800 dark:text-gray-100", children: task.text }), task.completionDate && (_jsx("span", { title: `Due in ${daysLeft} day${daysLeft === 1 ? "" : "s"}${!task.reminderEnabled ? " (reminder disabled)" : ""}`, className: `text-sm ${!task.reminderEnabled
                                                                                ? "text-gray-300 dark:text-gray-500 opacity-50"
                                                                                : daysLeft !== null && daysLeft <= 1
                                                                                    ? "text-red-500 dark:text-red-400 animate-pulse"
                                                                                    : "text-gray-400 dark:text-gray-500"}`, children: daysLeft !== null && daysLeft <= 1 ? (_jsx(BellRing, { size: 16 })) : (_jsx(Bell, { size: 16 })) }))] }) })] }), _jsx(Link, { to: `/event/${task.eventId}`, className: "text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 text-sm font-medium transition-all duration-200 hover:underline focus:ring-2 focus:ring-blue-400 focus:outline-none rounded", children: "View \u2192" })] }), _jsxs("div", { className: "text-sm text-gray-600 dark:text-gray-400 space-y-1", children: [_jsxs("div", { children: [task.owner ? `👤 ${task.owner}` : "👤 No owner", " • ", task.completionDate
                    ...
                    : 'bg-red-500 text-white'}`, children: _jsx("p", { className: "font-medium", children: toast.message }) })), _jsx("button", { onClick: () => navigate('/add'), className: "fixed bottom-6 right-6 bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-700 text-white rounded-full w-14 h-14 shadow-lg flex items-center justify-center text-3xl transition-all duration-200 hover:scale-110 active:scale-95 z-50 focus:ring-2 focus:ring-blue-400 focus:outline-none", title: "Add", "aria-label": "Add", children: "+" })] }));
}
