...
export default function Login() {
    ...
    if (user) {
        ...
        return (_jsx("div", { className: "flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-blue-100 to-purple-100", children: _jsxs("div", { className: "bg-white p-6 rounded-xl shadow-md w-80 text-center", children: [_jsxs("h1", { className: "text-xl font-semibold mb-4 text-gray-800", children: ["Welcome, ", displayName, "! \uD83D\uDC4B"] }), _jsx("button", { onClick: async () => {
                            await signOut();
                            navigate("/login");
                        }, className: "bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition w-full", children: "Logout" })] }) }));
    }
    return (_jsx("div", { className: "flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-blue-100 to-purple-100", children: _jsxs("div", { className: "bg-white p-6 rounded-xl shadow-md w-80", children: [_jsx("h1", { className: "text-xl font-semibold mb-4 text-center text-gray-800", children: isLogin ? "Welcome back" : "Create your account" }), _jsxs("form", { onSubmit: handleSubmit, className: "flex flex-col gap-3", children: [!isLogin && (_jsx("input", { type: "text", placeholder: "Your name", value: name, onChange: (e) => setName(e.target.value), required: true, className: "border border-gray-300 px-3 py-2 rounded text-sm" })), _jsx("input", { type: "email", placeholder: "Email", value: email, onChange: (e) => setEmail(e.target.value), required: true, className: "border border-gray-300 px-3 py-2 rounded text-sm" }), _jsx("input", { type: "password", placeholder: "Password", value: password, onChange: (e) => setPassword(e.target.value), required: true, className: "border border-gray-300 px-3 py-2 rounded text-sm" }), _jsx("button", { disabled: loading, className: "bg-blue-500 text-white py-2 rounded hover:bg-blue-600 transition", children: isLogin ? "Sign In" : "Sign Up" })] }), error && (_jsx("p", { className: "text-red-600 text-sm text-center mt-2", children: error.includes("already registered")
