...
const showNotification = (event) => {
    if (typeof globalThis !== 'undefined' &&
        ...
        new globalThis.Notification('⏰ Upcoming Event Reminder', {
...
const showTaskNotification = (task, event) => {
    if (typeof globalThis !== 'undefined' &&
        typeof globalThis.Notification !== 'undefined' &&
        globalThis.Notification.permission === 'granted') {
        const countdown = getCountdownObject(task.completionDate);
        new globalThis.Notification('⏰ Task Due Soon', {
...
const checkUpcomingTasks = (events) => {
    events.forEach((event) => {
        event.tasks.forEach((task) => {
            if (!task.completionDate || task.done || task.notified || !task.reminderEnabled)
                return;
            const countdown = getCountdownObject(task.completionDate);
...
export function createEventStore(storage) {
    const storageWrapper = {
        getItem: (name) => {
            ...
            if (v && typeof v.then === "function") {
                return v.then((value) => value ?? null);
            }
            return v ?? null;
        },
        setItem: (name, value) => {
            const r = storage.setItem(name, value);
            if (r && typeof r.then === "function")
                return r;
        },
        removeItem: (name) => {
            const fn = storage.removeItem?.bind(storage);
            const r = fn ? fn(name) : undefined;
            if (r && typeof r.then === "function")
                return r;
        }
    };
    return create()(persist((set, get) => ({
              ...
              `)
                ...
                if (data) {
                    ...
                    const transformedEvents = data
                        ...
                        const sharedWithArray = (moment.shared_with || []).map((e) => e.toLowerCase().trim());
                        ...
                        const isOwner = moment.user_id === userId;
                        if (isOwner)
                            return true; // Owners always see their moments
        ...
        shareMoment: async (momentId, emails) => {
            if (!supabase?.from) {
                return {
                    success: false,
                    count: 0,
            ...
            try {
                ...
                if (data) {
                                    ...
                                    if (notifError && notifError.code === '42501') {
                                        ...
                                        try {
                                            ...
                                            if (!url || !key) {
                                                ...
                                                const response = await fetch(`${url}/rest/v1/notifications?select=*`, {
                                                    method: 'POST',
                                                    headers: {
                    ...
                    set((s) => ({
                        events: s.events.map((m) => m.id === momentId
                            ? {
                                ...m,
                                // keep local shape; no strict type for shared_with on EventItem
                                ...(data.shared_with ? { shared_with: data.shared_with } : {}),
                                ...(data.shared_with_status ? { shared_with_status: data.shared_with_status } : {})
                            }
                            : m)
                    }));
        ...
        acceptMoment: async (momentId, userEmail) => {
            if (!supabase?.from) {
            ...
            try {
                ...
                const sharedWithArray = (momentData.shared_with || []).map((e) => e.toLowerCase().trim());
                ...
                const updateData = { shared_with_status: statusMap };
                if (!isOwner) {
        ...
        declineMoment: async (momentId, userEmail) => {
            if (!supabase?.from) {
        ...
        subscribeRealtime: () => {
            ...
            try {
                const channel = supabaseClient
                    ...
                    set((s) => ({
                        events: s.events.map((m) => m.id === updated.id
                            ? {
                                ...m,
                                ...
                                ...(updated.shared_with ? { shared_with: updated.shared_with } : {}),
                                ...(updated.shared_with_status ? { shared_with_status: updated.shared_with_status } : {}),
            ...
            if (supabase?.from) {
                try {
                    ...
                    if (updatedEvent.title)
                        supabaseUpdate.title = updatedEvent.title;
                    if (updatedEvent.description !== undefined)
                        supabaseUpdate.description = updatedEvent.description;
                    if (updatedEvent.date)
                        supabaseUpdate.date = updatedEvent.date;
                    if (updatedEvent.category)
                        supabaseUpdate.category = updatedEvent.category;
        ...
        toggleTask: (eventId, taskId) => set((state) => ({
            events: state.events.map((e) => e.id === eventId
                ? {
                    ...e,
                    tasks: e.tasks.map((t) => t.id === taskId
                        ? {
                            ...t,
        ...
        updateTask: async (eventId, taskId, updates) => {
            const updatedTask = {
                ...
                text: updates.text ?? updates.title
            };
            // Sync first
            if (supabase?.from) {
                try {
                    const { error } = await supabase
                        ...
                        done: updatedTask.done ?? updatedTask.completed,
        ...
        getAllTasks: () => {
            ...
            const fromEvents = state.events.flatMap((event) => event.tasks.map((task) => ({
                ...task,
                eventId: event.id,
            ...
            const fromFlatTasks = state.tasks || [];
            return fromFlatTasks.map((task) => ({
                ...task,
        ...
        getActiveTasks: () => {
            ...
            const tasksFromEvents = state.events
                .filter((m) => !(m.isPast || new Date(m.date) < today))
                .flatMap((event) => event.tasks.map((task) => ({
                ...task,
                eventId: event.id,
                eventTitle: event.title || event.name || "",
                eventCategory: event.category
            ...
            if (tasksFromEvents.length > 0)
                return tasksFromEvents;
            // Fallback to flat tasks array joined with moments by id
            const flat = state.tasks || [];
            const activeMomentIds = new Set((state.moments || state.events).filter((m) => new Date(m.date) >= today).map((m) => m.id));
            return flat
                .filter((t) => activeMomentIds.has(t.moment_id || t.moments?.id))
                .map((task) => ({
                ...task,
                eventId: task.moment_id || task.moments?.id || "",
                eventTitle: task.moments?.title || "",
        ...
        addComment: async (eventId, comment) => {
            ...
            // Fallback local-only
            const localComment = { ...comment, id: comment.id || generateId() };
            set((state) => ({
                events: state.events.map((e) => e.id === eventId
        ...
        updateReflection: async (momentId, reflection) => {
            ...
            set((state) => ({
                events: state.events.map((e) => e.id === momentId
        ...
        updateMoment: async (updatedMoment) => {
            ...
            set((state) => ({
                events: state.events.map((m) => m.id === updatedMoment.id ? { ...m, ...updatedMoment, isPast } : m),
                moments: (state.moments || []).map((m) => m.id === updatedMoment.id ? { ...m, ...updatedMoment, isPast } : m)
            }));
            if (supabase?.from) {
                try {
                    const payload = {};
                    if (updatedMoment.title !== undefined)
                        payload.title = updatedMoment.title;
                    if (updatedMoment.description !== undefined)
                        payload.description = updatedMoment.description;
                    if (updatedMoment.date !== undefined)
                        payload.date = updatedMoment.date;
                    if (updatedMoment.category !== undefined)
                        payload.category = updatedMoment.category;
