-- Supabase RLS Policies for Moments App with Collaboration Support
...
-- Policy: Users can view preparations for their own OR shared moments
CREATE POLICY "Users can view their own preparations"
ON public.preparations FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM moments WHERE moments.id = preparations.moment_id AND (moments.user_id = auth.uid() OR (auth.jwt()->>'email') = ANY (moments.shared_with))));

-- Policy: Users can insert preparations for their own OR shared moments
CREATE POLICY "Users can insert their own preparations"
ON public.preparations FOR INSERT TO authenticated
WITH CHECK (EXISTS (SELECT 1 FROM moments WHERE moments.id = preparations.moment_id AND (moments.user_id = auth.uid() OR (auth.jwt()->>'email') = ANY (moments.shared_with))));

-- Policy: Users can update preparations for their own OR shared moments
CREATE POLICY "Users can update their own preparations"
ON public.preparations FOR UPDATE TO authenticated
USING (EXISTS (SELECT 1 FROM moments WHERE moments.id = preparations.moment_id AND (moments.user_id = auth.uid() OR (auth.jwt()->>'email') = ANY (moments.shared_with))))
WITH CHECK (EXISTS (SELECT 1 FROM moments WHERE moments.id = preparations.moment_id AND (moments.user_id = auth.uid() OR (auth.jwt()->>'email') = ANY (moments.shared_with))));

-- Policy: Users can delete preparations for their own OR shared moments
...
-- Policy: Users can view comments for their own OR shared moments
CREATE POLICY "Users can view their own comments"
ON public.comments FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM moments WHERE moments.id = comments.moment_id AND (moments.user_id = auth.uid() OR (auth.jwt()->>'email') = ANY (moments.shared_with))));

-- Policy: Users can insert comments for their own OR shared moments
CREATE POLICY "Users can insert their own comments"
ON public.comments FOR INSERT TO authenticated
WITH CHECK (EXISTS (SELECT 1 FROM moments WHERE moments.id = comments.moment_id AND (moments.user_id = auth.uid() OR (auth.jwt()->>'email') = ANY (moments.shared_with))));

-- Policy: Users can delete comments for their own OR shared moments
